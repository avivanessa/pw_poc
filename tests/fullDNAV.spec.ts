...
test.describe('Valuation and Reconciliation > Home Page', () => {
    ...
    test('TC15 - Verify user is able to create new audit', async ({page}) => {
        const {fulldnav, sideMenuComponent} = initializePages(page)
        await sideMenuComponent.clickAuditDirectory()
        await fulldnav.createNewAudit(`${process.env.CLIENT}`,`${process.env.FISCAL_YEAR}`,`${process.env.ENGAGEMENT_ID}`,`${process.env.DATA_IMPORT_ID}`,`${process.env.OPINION_DATE}`)
        await fulldnav.verifyAuditCreated(`${process.env.CLIENT}`)
        // await page.pause()
    })
